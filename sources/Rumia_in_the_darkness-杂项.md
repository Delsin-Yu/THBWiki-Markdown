...
游戏的音乐、音效和地图图块文件主要来自Steam上的综合素材包《Kenney Game Assets》（目前已下架），其它的是RM引擎自带的默认素材（例如：猫叫声）。
  
  
露米娅的头像和行走图是开发者自行绘制的。
  
  
琪斯美的头像是使用Steam上的人物图像素材包《Character Creator - Graphics Pack》绘制的，行走图是在RM默认素材的基础上改造的。
