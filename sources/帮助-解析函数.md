...
本页是THBWiki的[编辑帮助文档](./分类-帮助文档.md)<center>
<table>
<tbody><tr>
<th>目录
</th></tr>
...
本页介绍本WIKI内大部分**解析函数**（**Parser Functions**，由**扩展插件**（**Extensions**[^cite_note-1]）通过`Parser::setFunctionHook`定义[^cite_note-2]）的用途和用法，扩展列表参照特殊:版本信息 (未找到链接)，MediaWiki原生函数基本参照[\[1\]](https://www.mediawiki.org/wiki/Help:Magic_words)。全部用法介绍均以**中文环境**为主，并侧重于全面介绍用法和易懂例子，不完全是其他介绍网页的翻译，不重要的废话可免则免。如有疑难可以加Q群**285426714**。
...
对于扩展插件开发者来说，可以通过对`Parser::setFunctionHook`传入各种参数稍微改变**解析函数**的引用格式：
  

- 可以去掉**函数**名前面的井号“#”，让格式更简单，更靠拢模板。

: 例如：`{{color: red | 文字 }}` → `文字`（[color用法](#color)）

- 可以禁止对传入的**参数**自动展开，这个设定非常有用，对于一些**参数**复杂、经常需要包含其他代码的**函数**来说，这样省了却大量的<nowiki></nowiki>。

: 例如：`{{#arraymap: 1,2,3 | , | @ | {{#expr: @+10}} | / }}` → `11/12/13`（[expr用法](#expr)）
...
: 这样可以：`{{#arraymap: 1,2,3 | , | @ | {{color: red | E-@}} | / }}` → `E-1/E-2/E-3`
: 这样不可以：`{{#arraymap: 13,14,19 | , | @ | 伊@ | | }}` → `伊13伊14伊19`（使用管道符作为输出分隔符，[arraymap用法](#arraymap)）
: 这样才可以：`{{#arraymap: 13,14,19 | , | @ | 伊@ | {{!}} }}` → `伊13|伊14|伊19`
...
: `{{#addlink:[沙盒]}}` → `[沙盒](./沙盒.md)`
: `{{#addlink:[[沙盒]]}}` → `[沙盒](./沙盒.md)`
: `{{#addlink:https://thwiki.cc/ THBWiki}}` → `[THBWiki](https://thwiki.cc/)`
: `{{#addlink:[[https://thwiki.cc/ THBWiki]]}}` → `[THBWiki](https://thwiki.cc/)`
...
把输入的字串转换成锚点编码，即是网址栏中#号后面的文字，可以用于生成锚点链接，但一般情况只需要`[[词条名#锚点]]`就行了。
...
: 
: `{{anchorencode: 字串 }}` → `字串`
: `{{anchorencode: ABC中文 英文 123&456+1% }}` → `ABC中文_英文_123&456+1%`
...
: 可以定义一个空数组。（见例子）
: 可以指定选项：`unique`，`sort`和`print`。（见例子）
: 如果分隔符没有被指定，选项会被忽略。
...
: `{{#arrayprint:b ||@ |@ |print=pretty }}` → 使用“漂亮列表输出”会把最后两个元素用`and`（英文时）、`和`（中文、日文时）连接。虽然分隔符参数为空，但是为了“漂亮”，还是会用半角逗号`,`（英文时）、顿号`、`（中文、日文时）作为间隔符。
...
例子
: 
: `{{#avatar: Arumi | 40px }}` → `<img height="40px" src="https://avatar.thwiki.cc/thwikicc_wiki_234_ml.jpg?r=1710270225" border="0" class="mw-socialprofile-avatar" loading="lazy" alt="avatar">`
: `{{#avatar: 睿域重工研究院 | s }}` → `<img src="https://avatar.thwiki.cc/thwikicc_wiki_5686_s.jpg?r=1690737711" border="0" class="mw-socialprofile-avatar" loading="lazy" alt="avatar">`
: `{{#avatar: 幻想旅人 | f }}` → `<img src="https://avatar.thwiki.cc/thwikicc_wiki_628_m.png?r=1690785431" border="0" class="mw-socialprofile-avatar" loading="lazy" alt="avatar">`
: `{{#avatar: Hency chine | ml }}` → `<img src="https://avatar.thwiki.cc/thwikicc_wiki_2036_ml.jpg?r=1690736194" border="0" class="mw-socialprofile-avatar" loading="lazy" alt="avatar">`
: `{{#avatar: Rinmoe | l }}` → `<img src="https://avatar.thwiki.cc/thwikicc_wiki_498_l.png?r=1690784853" border="0" class="mw-socialprofile-avatar" loading="lazy" alt="avatar">`
...
参见[帮助:SMW#calendarenddate](./帮助-SMW.md#calendarenddate)。
...
参见[帮助:SMW#calendarstartdate](./帮助-SMW.md#calendarstartdate)。
...
例子
: 
: `{{canonicalurl: 帮助:解析函数 }}` → `[https://thwiki.cc/%E5%B8%AE%E5%8A%A9:%E8%A7%A3%E6%9E%90%E5%87%BD%E6%95%B0](https://thwiki.cc/帮助:解析函数)`
: `{{canonicalurl: 帮助:解析函数 | action=edit }}` → `[https://thwiki.cc/index.php?title=%E5%B8%AE%E5%8A%A9:%E8%A7%A3%E6%9E%90%E5%87%BD%E6%95%B0&action=edit](https://thwiki.cc/index.php?title=帮助:解析函数&action=edit)`
...
例子
: 
: `{{canonicalurle: 帮助:解析函数 }}` → `[https://thwiki.cc/%E5%B8%AE%E5%8A%A9:%E8%A7%A3%E6%9E%90%E5%87%BD%E6%95%B0](https://thwiki.cc/帮助:解析函数)`
: `{{canonicalurle: 帮助:解析函数 | action=edit }}` → `[https://thwiki.cc/index.php?title=%E5%B8%AE%E5%8A%A9:%E8%A7%A3%E6%9E%90%E5%87%BD%E6%95%B0&action=edit](https://thwiki.cc/index.php?title=帮助:解析函数&action=edit)`
...
hideroot参数
: 若把此参数设为“on”（例`hideroot=on`），你输入的那个分类则不会被显示在列表中。
hideprefix参数
...
showcount参数
: 若把此参数设为“on”（例`showcount=on`），所有分类名旁边均会显示内含页面的数量。
namespaces参数
: 此参数用于按命名空间照筛选在列表中显示的页面，参数内容必须是一个以半形空格分隔的命名空间名称列表（例`namespaces=帮助 文件`），主空间可以用“-”或“0”表达。会覆盖mode参数的效果。
class、style、id等参数
: 此函数支持一般HTML属性，可以直接为列表添加class和style（例如让列表显示在右边可以用`style=float:right;`）。
格式
: `{{#categorytree: 分类名 | 参数1 | 参数2 | ... | 参数n }}`
例子
: 
: 这里用[分类:帮助文档](./分类-帮助文档.md)来做例子，因为该分类含有子分类，多个命名空间的词条和文件。
: `{{#categorytree:帮助文档|style=float:right;}}` → 效果显示在右边
  [帮助文档](./分类-帮助文档.md) [名称模板目录](./分类-名称模板目录.md) [规范文档](./分类-规范文档.md): `{{#categorytree:帮助文档|mode=parents}}` → 反向获取父分类
  [帮助文档](./分类-帮助文档.md)  [系统维护](./分类-系统维护.md): `{{#categorytree:帮助文档|depth=0}}` → 深度为0，点击分类前面的三角形展开。
  [帮助文档](./分类-帮助文档.md): `{{#categorytree:帮助文档|depth=0|hideprefix=never}}` → “never”看来是有bug的，仍然会隐藏所有前缀。
  [帮助文档](./分类-帮助文档.md): `{{#categorytree:帮助文档|depth=0|showcount=on}}` → 显示内容数量
  [帮助文档](./分类-帮助文档.md)‎ (2个分类、​57个页面、​62个文件): `{{#categorytree:帮助文档|namespaces=帮助 分类}}` → 现在只显示帮助和分类空间下的页面（这个例子中的首页和文件会被隐藏）
...
来自Chinesenumerals Extension，参见[帮助:汉字数字扩展#数字转汉字](./帮助-汉字数字扩展.md#数字转汉字)。  
...
来自Chinesenumerals Extension，参见[帮助:汉字数字扩展#汉字转数字](./帮助-汉字数字扩展.md#汉字转数字)。  
...
参见[帮助:SMW#concept](./帮助-SMW.md#concept)。
...
: `{{#count:i have a pen, i have an apple}}` → `7`
: `{{#count:a, b,c, d|<nowiki>, </nowiki>}}` → `2`
...
例子
: 
: `{{CREATEUSER: 沙盒 }}` → `JoesSR`
: `{{CREATEUSER: 首页 }}` → `MediaWiki default`
...
: `{{#cscore: Arumi | score }}` → `17,976`
: `{{#cscore: Arumi | changes }}` → `66,440`
: `{{#cscore: Arumi | pages }}` → `17,534`
...
参见[帮助:SMW#declare](./帮助-SMW.md#declare)。
...
设定词条默认在分类中的排序键，设定前默认为词条名，在添加分类时写`[[分类:分类名|排序键]]`可以覆写这个默认排序键。同一词条里有两次DEFAULTSORT的时候会报错，可以使用选项noerror来防止，noreplace则用来防止后面的DEFAULTSORT覆写前面的DEFAULTSORT。
...
例子
: 
: `{{#explode: 灵梦，魔理沙，早苗，正邪 |，| 2 }}` → `早苗`
: `{{#explode: 灵梦，魔理沙，早苗，正邪 |， | -1 }}` → `正邪`
...
此函数在Scribunto扩展中也可用，函数名为`mw.ext.ParserFunctions.expr`。
...
例子
: 当用于计算布尔代数值（[Boolean algebra](https://en.wikipedia.org/wiki/boolean_algebra_(logic%29)）时，零值表示`false（假）`，任何非零值，无论正负，都表示`ture（真）`：
: `{{#expr: 1 and -1 }}` → `1`
: `{{#expr: 1 and 0 }}` → `0`
: `{{#expr: 1 or -1 }}` → `1`
: `{{#expr: -1 or 0 }}` → `1`
...
: 输入空的表达式会返回一个空字串。输入不正确的表达式会返回其中一个错误消息，错误可以被[iferror](./帮助-解析函数.md#iferror)函数捕捉到：
: `{{#expr: }}` → ``
...
: 比如`{{NUMBEROFUSERS}}`输出的是`79,537`，但是实际需要的是`79537`，原始格式可以用`{{formatnum:{{NUMBEROFUSERS}}|R}}`来得到，详见[formatnum](./帮助-解析函数.md#formatnum)。例子如下：
: `{{#expr:{{NUMBEROFUSERS}}+100}}` → `**表达式错误：无法识别标点符号“,”。**`
: `{{#expr:{{formatnum:{{NUMBEROFUSERS}}|R}}+100}}` → `79637`

注意
: 运算符`mod`会在某些除数值下给出错误的结果：
: `{{#expr: 123 mod (2^64-1)}}` → `**除以零。**`（返回空字串，结果本应是`123`）

补充
: 若想要进行关于日期的计算（比如：将当前日期与某日期进行比较），首先应将时间转化为“1970年1月1日之后多少秒”的结构，可以使用`{{#time: xNU }}`来转换（详见[time](./帮助-解析函数.md#time)），然后就可以把日期当作数字来进行计算和比较了。
关于四舍五入
: 将运算符`round`左边的数四舍五入（[Rounds](http://php.net/manual/en/function.round.php)），使其小数位数为`round`右边的数。
: 如需进一取整（向上舍入），使用`ceil`；如需舍去取整（向下舍入），使用`floor`。
...
关于字符串
: 表达式只能用于数字值，不能比较字串或者字符。如需比较字串和字符，请使用[ifeq](./帮助-解析函数.md#ifeq)。例子：
...
例子
: 
: `{{fullurl: 帮助:解析函数 }}` → `[https://thwiki.cc/%E5%B8%AE%E5%8A%A9:%E8%A7%A3%E6%9E%90%E5%87%BD%E6%95%B0](https://thwiki.cc/帮助:解析函数)`
: `{{fullurl: 帮助:解析函数 | action=edit }}` → `[https://thwiki.cc/index.php?title=%E5%B8%AE%E5%8A%A9:%E8%A7%A3%E6%9E%90%E5%87%BD%E6%95%B0&action=edit](https://thwiki.cc/index.php?title=帮助:解析函数&action=edit)`
...
例子
: 
: `{{fullurle: 帮助:解析函数 }}` → `[https://thwiki.cc/%E5%B8%AE%E5%8A%A9:%E8%A7%A3%E6%9E%90%E5%87%BD%E6%95%B0](https://thwiki.cc/帮助:解析函数)`
: `{{fullurle: 帮助:解析函数 | action=edit }}` → `[https://thwiki.cc/index.php?title=%E5%B8%AE%E5%8A%A9:%E8%A7%A3%E6%9E%90%E5%87%BD%E6%95%B0&action=edit](https://thwiki.cc/index.php?title=帮助:解析函数&action=edit)`
...
: `{{#getmaparray:大战争音乐名/日文|<br />|pair}}` → `!CAT 音乐名!DEF 缺少参数!TEM 大战争音乐名1 春の氷精2 可愛い大戦争のリフレーン3 いたずらに命をかけて4 年中夢中の好奇心5 真夜中のフェアリーダンス6 妖精大戦争 ～ Fairy Wars7 ルーズレイン8 メイガスナイト9 春の氷精 - 静 -10 プレイヤーズスコアE 春の氷精 - 静 -P プレイヤーズスコアT 春の氷精`
...
例子
: 
: `{{#getparam:模板:同人专辑信息|,}}` → `制作方,制作方分隔符,展会,发行方,类型,编号,风格类型,特定选材,会场售价,通贩售价,售价补充,售价,售价说明,首发日期,非实参首发,名称,封面,封面角色,封面角色分隔符,译名,读音,发行方分隔符,原声集显示,原声集,原声集分隔符,重制,编号分隔符,年龄限制,会场售价补充,通贩售价补充,官网页面,官网页面分隔符,官网说明,官网补充,备注,碰撞规则`
...
对字串进行HTML解码，内部调用了`html_entity_decode`函数，FLAG使用了`ENT_QUOTES | ENT_XML1`。
...
对字串进行HTML编码，内部调用了`wfEscapeWikiText`函数，和后面带e的原生函数（如[pagenamee](./帮助-解析函数.md#pagenamee)）调用的是同一个函数。效果比htmlspecialchars复杂，从`wfEscapeWikiText`的原代码中可以看到替换了什么字，做了什么处理。
  

```
function wfEscapeWikiText( $text ) {static $repl = null, $repl2 = null;if ( $repl === null ) {$repl = array('"' => '"', '&' => '&', "'" => ''', '<' => '<','=' => '=', '>' => '>', '[' => '[', ']' => ']','{' => '{', '|' => '|', '}' => '}', ';' => ';',"\n#" => "\n#", "\r#" => "\r#","\n*" => "\n*", "\r*" => "\r*","\n:" => "\n:", "\r:" => "\r:","\n " => "\n ", "\r " => "\r ","\n\n" => "\n ", "\r\n" => " \n","\n\r" => "\n ", "\r\r" => "\r ","\n\t" => "\n ", "\r\t" => "\r ", // "\n\t\n" is treated like "\n\n""\n----" => "\n----", "\r----" => "\r----",'__' => '__', '://' => '://',);// We have to catch everything "\s" matches in PCREforeach ( array( 'ISBN', 'RFC', 'PMID' ) as $magic ) {$repl["$magic "] = "$magic ";$repl["$magic\t"] = "$magic ";$repl["$magic\r"] = "$magic ";$repl["$magic\n"] = "$magic ";$repl["$magic\f"] = "$magic ";}// And handle protocols that don't use "://"global $wgUrlProtocols;$repl2 = array();foreach ( $wgUrlProtocols as $prot ) {if ( substr( $prot, -1 ) === ':' ) {$repl2[] = preg_quote( substr( $prot, 0, -1 ), '/' );}}$repl2 = $repl2 ? '/\b(' . join( '|', $repl2 ) . '):/i' : '/^(?!)/';}$text = substr( strtr( "\n$text", $repl ), 1 );$text = preg_replace( $repl2, '$1:', $text );return $text;}```
...
: 这个函数可以被嵌套。嵌套时，需用完整格式的`#if`函数，替代外层`#if`函数的某个参数。最大可以嵌套七层，具体取决于网站和存储限制。
...
: `{{#ifeq: 10^3 | 1000 | equal | not equal}}` → `not equal` *（注意与上方使用了`#expr`函数的区别）*

注意
: 
: `#ifeq`和`#switch`的数值比较与使用`#expr`进行比较不同：
...
用于分辨输入字串，并返回两个结果中的一个。如果输入字串中含有HTML元素包含`class="error"`，则会被当作`true（真）`。  
`class="error"`能由其他解析函数，如[expr](./帮助-解析函数.md#expr)、[time](./帮助-解析函数.md#time)、[rel2abs](./帮助-解析函数.md#rel2abs)产生，模板（[Templates](https://www.mediawiki.org/wiki/Special:MyLanguage/Help:Templates)）错误，如循环、递归和其他解析器错误也能产生。
...
例子
: 此函数当页面存在时判定为`true（真）`，不论它是否包含内容，是否为可见空白页（包含元数据，如分类链接和魔术字（[Magic words](https://www.mediawiki.org/wiki/Special:MyLanguage/Help:Magic_words)），但不含可见内容），是否为空白页，是否为重定向页。
: 只有链接为红色（页面不存在，如`帮助:闷声发大财 (未找到链接)`）的页面，才会被判定为`false（假）`，包括曾经存在但已经被删除的页面。
: `{{#ifexist: 帮助:解析函数 | exists | doesn't exist }}` → `exists`
: `{{#ifexist: 帮助:闷声发大财 | exists | doesn't exist }}` → `doesn't exist`

: 自定义过的系统消息页（[System message](https://www.mediawiki.org/wiki/Special:MyLanguage/Help:System_message)）和由软件定义的特殊页（[Special pages](https://www.mediawiki.org/wiki/Special:MyLanguage/Help:Special_pages)）会被判定为`true（真）`。
: <img alt="" src="https://upload.thwiki.cc/thumb/d/db/%E6%80%BB%E4%B9%A6%E8%AE%B0.jpg/150px-%E6%80%BB%E4%B9%A6%E8%AE%B0.jpg" decoding="async" loading="lazy" width="150" height="209" class="thumbimage" srcset="https://upload.thwiki.cc/thumb/d/db/%E6%80%BB%E4%B9%A6%E8%AE%B0.jpg/225px-%E6%80%BB%E4%B9%A6%E8%AE%B0.jpg 1.5x, https://upload.thwiki.cc/thumb/d/db/%E6%80%BB%E4%B9%A6%E8%AE%B0.jpg/300px-%E6%80%BB%E4%B9%A6%E8%AE%B0.jpg 2x" data-file-width="609" data-file-height="850">
 [放大](./文件-总书记.jpg.md)总书记（喂: `{{#ifexist: 特殊:监视列表 | exists | doesn't exist }}` → `exists`
: `{{#ifexist: 特殊:用户查核 | exists | doesn't exist }}` → `exists` （因为安装了[CheckUser](https://www.mediawiki.org/wiki/Extension:Checkuser)）
: `{{#ifexist: MediaWiki:Copyright | exists | doesn't exist }}` → `exists` （因为MediaWiki:Copyright (未找到链接)页面被自定义过）

: 如果一个页面使用`#ifexist:`来检查目标页面是否存在，则该页面会出现在特殊:链入页面 (未找到链接)中目标页面的列表里。
: 所以如果本页面（帮助:解析函数）中存在`{{#ifexist:用户:睿域重工研究院}}`，则在特殊:链入页面/用户:睿域重工研究院 (未找到链接)中，会包含`帮助:解析函数`。
: 在使用了共享媒体存储库（Shared media repository）的wiki上，`#ifexist:`还可以用来检查文件是否被上传到指定的存储库，而不是wiki自身：
: `{{#ifexist: 文件:已经决定了.jpg | exists | doesn't exist }}` → `doesn't exist`
: `{{#ifexist: 文件:由你来当.jpg | exists | doesn't exist }}` → `doesn't exist`
: `{{#ifexist: 媒体:总书记.jpg | exists | doesn't exist }}` → `exists`

: 如果本地wiki已经创建了以上文件，则会显示`exists`。
: `#ifexist:`不对跨wiki链接起作用。
ifexist的限制
: `#ifexist:`被认为是一个“高开销解析函数”；每个页面只能包含有限个这样的函数（包括模板中含的函数）。当超过限制的数量时，任何超出数量的`#ifexist:`函数会被自动判定为`false（假）`，不论目标页面是否存在。并且页面会被添加至[分类:页面中有太多耗费的语法功能呼叫](./分类-页面中有太多耗费的语法功能呼叫.md)。各个追踪分类（[Tracking categories](https://www.mediawiki.org/wiki/Help:Tracking_categories)）的名字在不同语言的wiki内可能有所不同。
: 在一些应用场合下，可以使用CSS来模拟ifexist的效果，如使用选择器`a.new`（用于选择指向不存在页面的链接）或`a:not(.new)`（用于选择指向存在页面的链接）。更进一步，既然单个页面中高开销解析解析函数的数量限制是由[$wgExpensiveParserFunctionLimit](https://www.mediawiki.org/wiki/Special:MyLanguage/Manual:$wgExpensiveParserFunctionLimit)参数控制的，可以通过修改`LocalSettings.php`来满足使用要求。
...
: '表达式'的计算方法和上面[expr](./帮助-解析函数.md#expr)完全一样，可用的运算符也相同。运算结果会被当作一个布尔值。
例子
: 输入任何空表达式会判断为`false（假）`：
: `{{#ifexpr: | yes | no}}` → `no`

: 和上面[expr](./帮助-解析函数.md#expr)中所述相同：零值被判断为`false（假）`，任何非零值被判断为`true（真）`，所以这个函数的功能也可以用[ifeq](./帮助-解析函数.md#ifeq)和[expr](./帮助-解析函数.md#expr)来实现：
...
: `{{int: multiarraymap-desc }}` → `提供将多个数列转置显示的方法`
: `{{int: markedaspatrolledtext }}` → `$1 (未找到链接)的已选中版本已被标识为已巡查。`
: `{{int: markedaspatrolledtext | 首页 }}` → `[首页](./readme.md)的已选中版本已被标识为已巡查。`
: `{{int: pipe-separator }}` → ` | `
...
例子
: 
: `{{localurl: 帮助:解析函数 }}` → `/%E5%B8%AE%E5%8A%A9:%E8%A7%A3%E6%9E%90%E5%87%BD%E6%95%B0`
: `{{localurl: 帮助:解析函数 | action=edit }}` → `/index.php?title=%E5%B8%AE%E5%8A%A9:%E8%A7%A3%E6%9E%90%E5%87%BD%E6%95%B0&action=edit`
...
例子
: 
: `{{localurle: 帮助:解析函数 }}` → `/%E5%B8%AE%E5%8A%A9:%E8%A7%A3%E6%9E%90%E5%87%BD%E6%95%B0`
: `{{localurle: 帮助:解析函数 | action=edit }}` → `/index.php?title=%E5%B8%AE%E5%8A%A9:%E8%A7%A3%E6%9E%90%E5%87%BD%E6%95%B0&action=edit`
...
引用特定页面内已预先标记的内容段落。只要在目标页面内，把目标内容用`<section begin="段落名称" />`和`<section end="段落名称" />`包裹，就可以在任何页面中使用此函数引用该段内容。如果段落名称内不含空格等在XML语法中有特殊意思的字符，段落名称前后的半形引号则可略去。
...
引用特定页面内位于特定标题下的内容段落。此函数使用方法比[lst](#lst)简单，不需要使用section来定义段落，只需要使用标题（例如`== 标题 ==`）。此函数会引用指定的标题之后开始，下一个同级标题之前的结束的内容，并不会包含标题本身。另外可以设定结束子标题名称，让函数引用从子标题名称开始直到结束子标题名称之前的所有内容，此功能可以用于同时引用多个段落，只要遇到结束子标题，不管是不是同级标题也会立刻停止引用，并不会引用到结束子标题及其后的内容。
...
: `{{NUMBERINGROUP: confirm }}` → `162`
: `{{NUMBERINGROUP: sysop }}` → `14`
...
例子
: 
: `{{PAGESIZE: 官方角色列表 }}` → `16,004`
: `{{PAGESIZE: 官方角色列表 | R }}` → `16004`
...
: `{{PROTECTIONLEVEL: edit | 首页 }}` → `edit-mainpage`
: `{{PROTECTIONLEVEL: move | 首页 }}` → `edit-mainpage`
...
: `{{#redititle:Comic Market|，|link strip self}}` → `[Comic Market](./Comic_Market.md)，[Comiket](./Comic_Market.md)，[Comike](./Comic_Market.md)，[CM](./Comic_Market.md)，[コミックマーケット](./Comic_Market.md)，[コミケット](./Comic_Market.md)，[コミケ](./Comic_Market.md)，[コミ](./Comic_Market.md)`
...
搜索模式例子
: 
: `{{#regex:花妈 紫妹妹 八意大妈 八坂老太 二妹 | /\S+妹+/u }}` → `紫妹妹`
替换模式格式
: `{{#regex: 字串 | 正则表达式 | 替换内容 | 次数限制（默认为-1） }}`
替换模式例子
: 
: `{{#regex:花妈 紫妹妹 八意大妈 八坂老太 二妹 | /(\S*(妈{{!}}老)\S*)/ | <del>$1</del> | 2 }}` → ` ~~花妈~~  紫妹妹  ~~八意大妈~~  八坂老太 二妹`，把含有妈和老的称呼用删除线划掉，并限制只划前两个。
修饰符“e”的作用例子
: 
: `{{#regex: 生命：17，耐久：21，时间：59 | /(\d+)/ | {{#expr:$1+1}} }}` → `生命：**表达式错误：无法识别标点符号“$”。**，耐久：**表达式错误：无法识别标点符号“$”。**，时间：**表达式错误：无法识别标点符号“$”。**`
: 一般来说里面的函数会先取值，然后再输入到外面的函数里，因为此例子中替换内容中含有函数，该函数（[expr](#expr)）会先被取值，变成`{{#expr:$1+1}}`，expr无法处理$1这个值，就只能报错了。
使用修饰符“e”后的例子
: 
: `{{#regex: 生命：17，耐久：21，时间：59 | /(\d+)/e | {{#expr:$1+1}} }}` → `生命：18，耐久：22，时间：60`
: 由于使用了修饰符“e”，替换内容中的函数会在替换后才取值，regex会把匹配到的字串（在这里是数字）放到$1的位置，然后才一次过对`{{#expr:17+1}}`等取值。
...
- 这里用`{{#regex:芙兰朵露 斯卡雷特（二妹，二小姐） | /(\S+) (\S+)（(.*?)）/u }}` → `芙兰朵露 斯卡雷特（二妹，二小姐）`作为例子。

: `{{#regex_var: 1 | 名字 }}` → `芙兰朵露`
: `{{#regex_var: 2 | 姓氏 }}` → `斯卡雷特`
: `{{#regex_var: 3 | 称呼 }}` → `二妹，二小姐`
: `{{#regex_var: $1·$2，又可称为$3 | 介绍 }}` → `芙兰朵露·斯卡雷特，又可称为二妹，二小姐`
...
例子
: 
: `{{#regexall:花妈 紫妹妹 八意大妈 八坂老太 二妹 | /\S+妹+/u | ， }}` → `紫妹妹，芙兰妹妹`
: `{{#regexall:花妈 紫妹妹 八意大妈 八坂老太 二妹 | /(\S*(妈{{!}}老)\S*)/ | ， | 1 | 2 }}` → `八意大妈，八坂老太`
...
: 如果'基础路径'参数没有被指定，则会使用当前页面的完整页面名称（可使用`{{FULLPAGENAME}}`获得，本页为`帮助:解析函数`）。
例子
: 一般情况如下：
...
例子
: 
: `{{#replace:Clown piece}}` → `Clownpiece`
: `{{#replace:Double_Dealing_Character|_|<nowiki> }}</nowiki>` → `DoubleDealingCharacter`
...
例子
: 
: `{{#replaceset:紫老太年龄9223372036854775807岁|老太=妹妹|/\d+岁/=17岁}}` → `紫妹妹年龄17岁`
: `{{#replaceset:上下 啊 逆转|啊=不能|/(\S{2})/i=\1...}}` → `上下... 不能... 逆转...`
: 替换规则2是在连续刚好两个非空字符后加上...，原本“啊”字并不符合这个条件，但由于替换规则1把“啊”替换成了“不能”，替换规则2就能作用到“不能”上了。
: `{{#replaceset:上下 啊 逆转|/(\S{2})/i=\1...|啊=不能}}` → `上下... 不能 逆转...`
: 替换规则1和2换个顺序的结果。
...
例子
: 
: `{{REVISIONTIMESTAMP: 词条名 }}` → `20150903184025`
...
参见[帮助:SMW#set_recurring_event](./帮助-SMW.md#set_recurring_event)。
...
例子
: 
: `{{#special: 特殊页面名称 }}` → `特殊:特殊页面`
: `{{#special: specialpages }}` → `特殊:特殊页面`
: `{{#special: userlogin }}` → `特殊:用户登录`
...
例子
: 
: `{{#speciale: 特殊页面 }}` → `%E7%89%B9%E6%AE%8A:%E7%89%B9%E6%AE%8A%E9%A1%B5%E9%9D%A2`
: `{{#speciale: specialpages }}` → `%E7%89%B9%E6%AE%8A:%E7%89%B9%E6%AE%8A%E9%A1%B5%E9%9D%A2`
: `{{#speciale: userlogin }}` → `%E7%89%B9%E6%AE%8A:%E7%94%A8%E6%88%B7%E7%99%BB%E5%BD%95`
...
: 如同[len](./帮助-解析函数.md#len)一样，`<nowiki>`等标签扩展，不计长度，也不占'位置'，如：
...
参见[帮助:SMW#subobject](./帮助-SMW.md#subobject)。
...
: 效果相等于在每个词条名格式为“帮助:解析函数/XXX”的词条写上`[[分类:扩展帮助文档|^解析函数^]]`。
...
: `#switch`和部分转换标签可以作用于配置文件，从而可以使不熟悉模板代码的编辑者编辑可配置元素。
关于默认结果
: 
...
: 或者'默认结果'可以用'情况字串'`#default`来特别声明。
...
关于比较
: 
: 与`#ifeq`相同，如果'比较字串'和'情况字串'都为数值，则会按照数值进行比较。以上情况外，区分大小写：
...
: `{{#tag: ref | 这是备注里面，{{padleft:|5|一二}}切运算都有效 | name="tag" group="tag" }}` → `[^cite_note-tag&quot;_group=&quot;tag-4]`
: `{{#tag: references || group="tag" }}` → ``（内文连一个空格都没有的话，便会生成一个自关闭标签）
...
<tr>
<td style="text-align: center;"><code>o</code><span>[^cite_note-5]</span>
</td>
<td>ISO-8601 规定的该周所属年份。<span>[^cite_note-6]</span>
</td>
<td style="text-align: center;">2024<span>[^cite_note-7]</span>
</td></tr>
<tr>
<td colspan="3" style="font-size: 88%;">
<div class="mw-references-wrap"><ol class="references">
<li id="cite_note-5"><span class="mw-cite-backlink"><a href="#cite_ref-5">↑</a></span> <span class="reference-text">需要 PHP 5.1.0 或更新版本和 MediaWiki <a class="external text">rev:45208</a>。</span>
</li>
...
可自行指定日期/时间对象，默认使用魔术字（[Magic word](https://www.mediawiki.org/wiki/Special:MyLanguage/Help:Magic_words#Date_and_time)）`{{CURRENTTIMESTAMP}}`的值（该值使用世界协调时间，UTC），也就是页面最近一次渲染为HTML的时间。
...
: 另外，还可以用`xx`来输出单个字母`x`。
例子
: `{{#time:\m}}` → `m`
: `{{#time:\xg}}` → `x8` （`x`被原样输出，`g`被当作格式控制代码）
: `{{#time: Y-m-d }}` → `2024-06-22`
: `{{#time: [[Y]] m d }}` → `2024 (未找到链接) 06 22`
: `{{#time: [[Y (year)]] }}` → `2024 (24UTCpmSat, 22 Jun 2024 20:38:23 +0000) (未找到链接)` （year被当作格式控制代码`y`、`e`、`a`、`r`）
: `{{#time: [[Y "(year)"]] }}` → `2024 (year) (未找到链接)`
: `{{#time: i's" }}` → `38'23"`
: '日期/时间对象'可以使用任何PHP中[strtotime()](http://www.php.net/manual/en/function.strtotime.php)函数支持的格式。可以使用绝对时间（如`20 December 2000`）和相对时间（如`+20 hours`）：
: `{{#time: r|now}}` → `Sat, 22 Jun 2024 20:38:24 +0000`
: `{{#time: r|+2 hours}}` → `Sat, 22 Jun 2024 22:38:24 +0000`
: `{{#time: r|now + 2 hours}}` → `Sat, 22 Jun 2024 22:38:24 +0000`
...
: `{{#time: U | now }}` → `1719088704`
: `{{#time: r|@1485582296}}` → `Sat, 28 Jan 2017 05:44:56 +0000`
: **注意：****可接受的输入时间范围**是**0111年1月1日**至**9999年12月31日**。100年到110年的各个格式控制代码的输出不一致：`Y`和`L`按照100-110年来计算；但`r`、`D`、`l`和`U`是按照2000-2010年来计算的：
...
: 超过9999年会得到不正确的结果：
: `{{#time: d F Y | 15 April 10000 }}` → `**错误：时间无效。**`
...
:  ~~星期名称支持100-110年和1753年以后的年份。111-1752年的`r`中星期输出为`Unknown`，`l`的输出为`<>`，且这几年的`r`输出值不能当作有效的输入值。~~ 
: **译者注：**: 测试了一些111-1752之间的年份，已经可以正常显示星期，且正确：
...
: `{{#time: Y m d H:i:s | June }}` → `2024 06 22 00:00:00` （使用了一天的开始时刻，填补了当前月份内的日期号、年份）
: `{{#time: Y m d H:i:s | 2003 }}` → `2003 06 22 00:00:00` （使用了一天的开始时刻，填补了当前年份内的日期号）


: 在MediaWiki [r86805](https://www.mediawiki.org/wiki/Special:Code/MediaWiki/86805)之后，4位数字一概会被认为是年份，而非小时与分钟：
: `{{#time: Y m d H:i:s | 1959 }}` → `1959 06 22 00:00:00`

: 6位数字会被认为是小时、分钟和秒钟，如果得到的时间不正确，则会报错（一概不会当作年份和月份）：
: `{{#time: Y m d H:i:s | 195909 }}` → `2024 06 22 19:59:09` （被当作时分秒而不是年月）
: `{{#time: Y m d H:i:s | 196009 }}` → `**错误：时间无效。**` （即使19:60:09不是正确的时间，196009也不会被当作年月）

: 函数定量计算天数来得到日期：
: `{{#time: Y F d | January 0 2008 }}` → `2007 12月 31`
: `{{#time: F d | January 32 }}` → `**错误：时间无效。**` （一个月超过31天的话，会报错）
: `{{#time: F d | February 29 2008 }}` → `2月 29`
: `{{#time: F d | February 29 2007 }}` → `3月 01`
: `{{#time:Y F|now -1 months}}` → `2024 5月`

: `#time`中'格式字串'的总长度限制为[6000字符](https://phabricator.wikimedia.org/diffusion/EPFN/browse/master/ParserFunctions_body.php$7)。
关于时区
: 解析函数`#time`中有一个bug（更确切的说是PHP日期时间的bug），不允许传入非整数的相对时差。这个问题在整点时区不会出现，如东部夏令时间（EDT）：
: `{{#time:g:i A | -4 hours }}` → `4:38 PM`

: 但是，委内瑞拉（委内瑞拉标准时间，VET）与世界协调时间（UTC）的时差是-4.5小时，如果直接输入则不会得到正确的结果，如下：
: `{{#time:g:i A | -4.5 hours }}` → `5:38 AM`

: 可以将时差转换为分钟或者秒钟来解决这个问题：
: `{{#time:g:i A | -270 minutes }}` → `4:08 PM`
: `{{#time:g:i A | -16200 seconds }}` → `4:08 PM`
...
此函数与`{{#time: ... }}`一致，只是它在'日期/时间对象'留空时，会使用wiki的当地时间（在[$wgLocaltimezone](https://www.mediawiki.org/wiki/Manual:$wgLocaltimezone)中设定）。
  
例子
: `{{#time: Y-m-d }}` → `2024-06-22` （世界协调时间，UTC）
: `{{#timel: Y-m-d }}` → `2024-06-23` （中央标准时间，CST）
: `{{#time: Y F d H:i:s}}` → `2024 6月 22 20:38:23` （世界协调时间，UTC）
: `{{#timel: Y F d H:i:s}}` → `2024 6月 23 04:38:23` （中央标准时间，CST）
...
例子
: 
: `{{#titleparts: 讨论:东方/心绮楼/Hopeless/Masquerade }}` → `讨论:东方/心绮楼/Hopeless/Masquerade`
: `{{#titleparts: 讨论:东方/心绮楼/Hopeless/Masquerade | 1 }}` → `讨论:东方`，效果同[rootpagename](#rootpagename)。
: `{{#titleparts: 讨论:东方/心绮楼/Hopeless/Masquerade | 2 }}` → `讨论:东方/心绮楼`
: `{{#titleparts: 讨论:东方/心绮楼/Hopeless/Masquerade | 2 | 2 }}` → `心绮楼/Hopeless`
: `{{#titleparts: 讨论:东方/心绮楼/Hopeless/Masquerade | | 2 }}` → `心绮楼/Hopeless/Masquerade`
注意
: 段落长度和段落位置均可以使用负数，负数段落长度表示从尾开始去掉该数量的段落，负数段落位置表示从尾开始计算段落位置。
例子
: 
: `{{#titleparts: 讨论:东方/心绮楼/Hopeless/Masquerade | -1 }}` → `讨论:东方/心绮楼/Hopeless`，效果同[basepagename](#basepagename)。
: `{{#titleparts: 讨论:东方/心绮楼/Hopeless/Masquerade | -4 }}` → ``，去掉的段落数等于段落总数。
: `{{#titleparts: 讨论:东方/心绮楼/Hopeless/Masquerade | -5 }}` → ``，去掉的段落数超出了段落总数。
: `{{#titleparts: 讨论:东方/心绮楼/Hopeless/Masquerade | | -1 }}` → `Masquerade`，效果同[subpagename](#subpagename)。
: `{{#titleparts: 讨论:东方/心绮楼/Hopeless/Masquerade | -1 | 2 }}` → `心绮楼/Hopeless`
: `{{#titleparts: 讨论:东方/心绮楼/Hopeless/Masquerade | -1 | -2 }}` → `Hopeless`
注意
...
: `{{#titleparts: [[沙盒]]/123 | 1 | 2 }}` → `[沙盒](./沙盒.md)/123`
...
: `{{#urldecode: ABC%E4%B8%AD%E6%96%87+%E8%8B%B1%E6%96%87+123%26456%2B1%25 }}` → `ABC中文 英文 123&456+1%`（用urlencode加QUERY编码的字串）
: `{{#urldecode: ABC%E4%B8%AD%E6%96%87_%E8%8B%B1%E6%96%87_123%26456%2B1%25 }}` → `ABC中文_英文_123&456+1%`（用urlencode加WIKI编码的字串）
: `{{#urldecode: ABC%E4%B8%AD%E6%96%87%20%E8%8B%B1%E6%96%87%20123%26456%2B1%25 }}` → `ABC中文 英文 123&456+1%`（用urlencode加PATH编码的字串）
...
在页面中获取网址参数，使用`{{#urlget: name|n/a}}`可以获取网址参数中name的值（例如网址是[thwiki.cc/帮助:解析函数?name=zun](./帮助-解析函数.md)，函数则会返回“zun”），如果网址中没有该参数，则返回默认值。结果值会保持未URL解码的状态下返回，返回默认值时该默认值也会被URL编码以保持统一，urlget外套一个[#urldecode](#urldecode)便可以获取真实的值。参数名中使用“A[B]”的方式可以获取阵列参数中的单个值，否则使用“A”的话只能获取以半形逗号分隔的整个阵列。注意使用此函数会自动禁用该页面的内部缓存功能。
...
- 假设网址是[https://thwiki.cc/帮助:解析函数?name=zun&love\[\]=beer&love\[\]=stg&love\[\]=music](./帮助-解析函数.md)。

: `{{#urlget: name | what??? }}` → `zun`
: `{{#urlget: love | none }}` → `beer,stg,music`，获取整个阵列。
: `{{#urlget: love[0] | none }}` → `beer`，获取阵列中的单个值。
: `{{#urlget: hate | what??? }}` → `what%3F%3F%3F`，注意“???”被编码了。
...
获取特定变量的内容值，需配合[#vardefine](#vardefine)或[#vardefineecho](#vardefineecho)使用，比如在这里用`{{#vardefine: abc | def }}`定义变量abc。无法获取在后面代码中才定义的变量。当要获取的变量没被定义时，会输出默认值，已被定义的判定可参考[#varexists](#varexists)。
  
格式
: `{{#var: 变量名称 | 默认返回值（默认为空字串） }}`
例子
: 
: `{{#var: count | 未定义 }}` → `未定义`获取未定义的变量count，输出默认值。
: `{{#var: abc | xyz }}` → `def`获取已定义的变量abc。
...
比如这里的代码位于[#vardefine](#vardefine)下的例子前，如果现在获取`{{#var:count}}`的话，因为定义count的代码还没处理，所以只能获取到变量未定义时默认的空白字串“”；或者使用`{{#var:count|未定义}}`的话，则返回“未定义”。  
...
例子
: 
: `{{#var_final: count | 未定义 }}` → `1`在此处获取在后面代码中才会被定义的count的内容。
...
例子
: 
: `{{#vardefine: count | 0 }}` → ``将定义一个名为“count”的变量，初始内容为“0”，需注意此为字串“0”，现在变量count的内容为“0”。
: `{{#vardefine: text |}}` → ``也可以定义初始内容为空白的变量，现在变量text的内容为“”。
: `{{#vardefine: text | 新 }}` → ``通过同样的方法可以覆盖已有变量的内容，现在变量text的内容为“新”。
: `{{#vardefine: text | {{#var: text}}内容 }}` → ``通过与[#var](#var)组合，给变量拼接更多内容，现在变量text的内容为“新内容”。
: `{{#vardefine: count | {{#expr: {{#var: count}}+1 }} }}` → ``通过与[#expr](#expr)组合，递增变量，现在变量count的内容为“1”。
...
例子
: 
: `{{#vardefineecho: text | 变量的内容，看到了吗 }}` → `变量的内容，看到了吗`定义变量同时输出变量内容，现在变量text的内容为“变量的内容，看到了吗”。
...
: `{{#varexists: count }}` → `1`
: `{{#varexists: null }}` → ``
: `{{#if: {{#varexists: extra }} | 存在 | 不存在 }}` → `不存在`配合[#if](#if)使用。
...
[^cite_note-2]: [https://www.mediawiki.org/wiki/Manual:Parser_functions](https://www.mediawiki.org/wiki/Manual:Parser_functions)
[^cite_note-3]: [https://www.w3schools.com/html/html_entities.asp](https://www.w3schools.com/html/html_entities.asp)
[^cite_note-tag&quot;_group=&quot;tag-4]:  这是备注里面，一二一二一切运算都有效 
