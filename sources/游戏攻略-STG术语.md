...
也简称**rep**。东方特有的录像文件（.rpy），可以在游戏中进行直接播放录像。录像文件（以及玩家数据等）可以在[replay文件夹内](./官方游戏-错误解决办法.md#用户数据（rep、分数等）的保存位置)找到，一般为th6_01.rpy形式。
  
  
中文版和日文版游戏生成的录像可能会不通用，需要注意。请参考[相关词条](./官方游戏-错误解决办法.md#录像文件相关)。
...
- User录像在红魔乡、妖妖梦、永夜抄、花映塚的Replay选择列表左边的No.栏中，统一显示为 "User" 。 
  - 而在其他作品中，则会显示前面所提到的命名规则中的 ???? 部分。比如 *th13_ud5678.rpy* 文件，会在神灵庙中显示为 **5678**。 
    ...
    - 在弹幕天邪鬼和秘封噩梦日记的Replay选择界面中，User录像的文件名显示有误：第一个字符永远显示为一个固定字符 **d**，而文件名中的最后一个字符却无法显示。具体参见 [官方游戏/错误解决办法](./官方游戏-错误解决办法.md#弹幕天邪鬼)。
...
随机体与夜点收集量不同，决死结界的持续时间也不同，详见词条[决死结界](./游戏攻略-东方永夜抄-其他.md#决死结界)。
...
<td>999999<span>[^cite_note-18]</span>
</td>
...
- [无意识「弹幕的墨迹测验」](./弹幕的墨迹测验.md)（可被击破，但符卡奖励分数不减，全避也可取得符卡奖励分数但是不掉落道具）
...
- “Cherry”右边用“／”分隔的两组数值，分别为“樱点值”和“最大樱点值”。当前樱点值即为得分道具的最大值（所谓的“最大得点”），最大樱点值则限制了这个值的最高值。
- +号右边的粉红色数值为“樱点Plus（樱点+）”，由于有加号俗称“+C”。通常所说的“樱点”一般指这个。
...
樱点与结界的详细说明可参考[东方妖妖梦系统说明页面](./游戏攻略-东方妖妖梦-系统.md#特殊系统)。
...
信仰值的详细增加与减少机制可参考[东方风神录系统说明页面](./游戏攻略-东方风神录-系统.md#信仰)。
  

### Hint
  
[风神录](./东方风神录.md)的特有功能，用于在游戏画面生成文本提示，可在Option里开启和关闭。
...
[星莲船](./东方星莲船.md)的主要系统。通过收集飞碟碎片而开启。
...
实际上，神灵庙与辉针城主程序中保留了Overdrive难度的全关与EX，只是正常情况下无法游玩。使用[overdrive难度解锁补丁](./游戏攻略-STG常用工具.md#改版游戏)可以进行解锁，属于改版游戏范畴，不是常规游戏。
...
- 春：解放的无敌时间比其他略长（1.5s，甚至长于解放冷却时间）。随充能槽的增加，解放的半径会大幅度提高。夏：每次只消耗1个充能。但是充能槽的等级几乎不影响性能，无敌时间也极短（约0.15s）。秋：解放的持续时间略微延长，提高自机移动速度，且消弹圈会跟随自机移动。冬：解放的持续时间延长，持续时间内自机的射击伤害会提高。土用：只有EX关卡可用。性能和夏类似，但是消弹圈会向正下方移动。季节解放的具体数据可以参考[天空璋说明](./游戏攻略-STG机体说明-东方天空璋.md)。
...
有些在固定发射角度、排列随机（例如[地灵殿5面道中猫步](./Cat's_Walk.md)，圈是固定位置的，但是圈上排列的针弹位置随机）。虽然通常也用“随机”来描述，但属于有一定规律的随机弹，在相对稀疏的位置躲避会事半功倍。
...
弹幕会在屏幕上固定位置出现、以固定形式移动的弹幕称为**纯固定**。例如[六道剑「一念无量劫」](./一念无量劫.md)就是标准的纯固定。不管打多少次，弹幕都是同一个样子。  

展开样式相同，但是会随boss的移动改变整体位置的弹幕，称为**boss依存**的固定弹。例如[村纱水蜜](./村纱水蜜.md)的非符。单个子弹的运动方式相对于这个弹幕整体而言是固定的。  

由于boss的移动，导致以boss与自机的连线为基准改变整体位置的弹幕，则称为**自机依存**的固定弹。例如[绀色的狂梦](./绀色的狂梦.md)、[怪力乱神](./怪力乱神.md)、风神录[早苗](./东风谷早苗.md)的道中非道中符等。如果自机与boss的相对位置相同，那么所面对的弹幕也会是相同的；如果boss不会动则可以打成纯固定。  

将一些固定形状的弹幕（或将其复制很多份）以随机的方向射出，则称为**形状固定、角度随机**，或是**角度随机的固定弹**（角随固）。例如[阿波罗13](./Apollo_13.md)、[奥尔良人偶](./博爱的法兰西人偶.md)、[神德](./Mountain_of_Faith.md)等。也属于有一定规律的随机弹，找寻相对固定的形状是基本解法。
...
弹链（ワインダー）
: 连续发射出弹与弹间隔较小的弹幕、或者以此形成墙壁状的封位弹幕（如[「永夜归返　-待宵-」](./永夜归返.md)） 。  
也有设计成需要穿过弹链的弹幕，此时的弹链间隔会略宽一点（如[塞符「天上天下的照国」](./山神渡御.md)）。
n-way弹
...
而花映塚由于游戏方式不同，rank是主系统的一部分，与一般射击游戏的rank系统差距较大，可参见[花映塚的rank系统](./游戏攻略-东方花映塚术语.md#Rank)。
...
东方STG中常用的安定可参考[游戏攻略/STG安定总结#官作安定总结页面](./游戏攻略-STG安定总结.md#官作安定总结页面)。
...
9961、9760、8888
: 东方红魔乡游戏中，如果玩家在终符[「红色的幻想乡」](./Red_Magic.md)疮痍，在结算界面中，“游戏进行率”选项会是99.61%；在6面道中非或符（[奇术「Eternal Meek」](./Eternal_Meek.md)）疮痍，进行率为88.88%；在关底4非或4符（[红符「Scarlet Meister」](./Scarlet_Shoot.md)）疮痍，此时进行率为97.60%。  
...
[^cite_note-2]: 确实是后期，出现弹幕系的时候街机STG几乎已经凉了。
[^cite_note-3]: 简单说，除了特殊系统（神灵庙、天空璋、虹龙洞）或miss触发的无敌之外，不允许使用无敌时间强行输出boss的打法。
[^cite_note-4]: ZUN本身对此也持不支持态度，在各作的网页版manual里都有这么一个Q&A。  
Ｑ．一旦按下暂停键画面就会变得模糊而看不清楚  
就是看不清楚，基本上，除了要停止游戏，请不要怀着其他目的按下暂停键。  
我觉得在射击当中按下暂停键，就跟玩猜谜游戏时这么作一样是一种邪道。
[^cite_note-5]: 风神录以后为出现续关选项“继续游戏/继续挑战 Continue”，th12.8~th14为“再挑战 Retry”
[^cite_note-6]: 绀珠传、兽王园、永夜抄的6A线除外。
[^cite_note-7]: 如[封魔录说明书](./附带文档-东方封魔录-Manual.md)的操作方法章节，有“连续按z会提高一定的连射力”这样的说法。
[^cite_note-8]: 特殊：地灵殿魔理沙-爱丽丝组合机体最大火力为8.00。
[^cite_note-9]: slow mode从花开始就取消了，但花这作确实可以调残到5残起飞。
[^cite_note-10]: 默认为3，游戏开始后右上角为两颗红星/心，没有0残起飞的说法
[^cite_note-11]: 实际上起飞以后所说的残一般指的都是红星/心数目，因此才有了0残通关这样的说法。
[^cite_note-12]: 永夜抄中，任意符卡和非符在击破时都会给予自机大约一秒的无敌时间，所谓的击破撞大多都是撞了之后才击破的。
[^cite_note-13]: 按ZUN的说法，这个圆点其实不是判定点，而是表现自机正在蓄力集中射击，因此圆点的视觉大小和实际判定并不一致。
[^cite_note-14]: 红魔乡和妖妖梦中，只有满P状态才会激活回收线，非满P无法上线收点(妖妖梦EX和PH不需要满P)；永夜抄中，只有满P或者处于低速状态时，才会激活回收线，实际上上线敲一下shift就能回收了(魔理沙、咏唱组机体不需要满P也不需要低速，注意单爱丽丝机体没有该效果)。其他作品没有限定条件。
[^cite_note-15]: 辉针城中的回收线位置显示有误，实际高度仍然是1/4。
[^cite_note-16]: 风神录的对话不回收道具使得弑神炮NN成为可能。在地灵殿中，进入对话时，信号槽将保持在进入对话前的状态，不会改变。鬼形兽中，进入对话时动物灵仍然正常移动变化，可以借此机会吃灵。虹龙洞中，进入对话会强制删除道具的季节卡牌召唤的莉莉白，拿不到她携带的道具，进入对话还会强制删除所有杂鱼，并触发买路财的每个杂鱼被击破额外掉落1资金点的效果，但原本携带的道具不会掉落，且不会自动回收资金点。
[^cite_note-17]: 所谓“被弹bomb”实为[红魔乡年代](./附带文档-东方红魔乡-Omake.md)“喰らいボム”的误译与误传。“被弾”其实就是中弹，日本东方STG领域并没有“被弾ボム”的说法。
[^cite_note-18]: 虽然游戏自带的说明文件中写的是 99999，但实际是 999999 。
[^cite_note-19]: 东方花映塚和的正式版有九面，然而体验版仍然只有前三面（东方兽王园体验版也只有三面），这算得上是ZUN的执念。（即体验版卖300日元，一个100日元硬币能打一关，是模仿街机投币的形式，你投3个币，ZUN让你打3关）
[^cite_note-20]: 实际上Boss受到的伤害不一定真的降低，具体要看程序代码怎么写的；但可以确定的是，Boss释放符卡后的那一节HP条下降速度在视觉上会变慢，造成一种“伤害降低了”的场面。
[^cite_note-21]: 6面最后两张符卡中，主动灵击并不会收卡失败。
[^cite_note-22]: 体验版中，使用春副子机的季节解放会导致收卡失败
[^cite_note-23]: 在咆哮模式结束，还未出现随机动物灵之前，又收集了包括稀有动物灵在内的5个灵，则此时也会掉出5个灵。属于bug。
[^cite_note-24]: 有些时候，敌人会根据自机的位置，使用不同种类的弹幕（如站在一个位置是散射、另一个位置则是高速密集弹，以及护身弹等），这种情况一般不称为狙。
[^cite_note-25]: 通常而言，只要发射方向瞄准自机则称为精确狙（比如发弹点移动或发弹点在一定范围内随机取得的弹幕。）
[^cite_note-26]: 风其实也有rank系统，但是几乎没有任何作用……
[^cite_note-27]: “ベントラー（ventra）”是美国人的George Van Tassel主张的“宇宙语中，表示宇宙飞船的词”，日本的“宇宙友好协会”（ufo爱好组织）使用该词，形成了“大声连着喊ベントラー！ベントラー！来召唤飞碟”的日本传统（和国人使用“飞碟”这个词类似）。ZUN在[星莲船manual文档](http://omake.thwiki.cc/manual/th12/ja/)里也这么玩过。
[^cite_note-28]: 这个说法最开始来自某人贬义的“猴智未开”，后来有好事者写了[猴力学概论](https://tieba.baidu.com/p/3044396412)。
