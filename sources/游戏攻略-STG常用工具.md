...
注意，这些工具和补丁均非官方出品，且通常不作**任何**保证。使用前请备份重要文件（例如玩家数据文件 `score.dat` 等）。  
...
全开档（提取码：7q45）
: 更新至TH17，包括开启EX关，以及各单关练习模式的存档。替换掉对应作品的 `score.dat` 即可生效。
...
RetroArch 方案
: 配置指南：[2024年，您还在使用Anex86玩东方旧作吗？](https://www.bilibili.com/opus/894274300553461760)，[第三方整合包](https://pan.baidu.com/s/1-eMIi4JIjyiTb8akg9r1Tw?pwd=9961)（仅 Windows，提取码：`9961`）
...
- [Zophar 的 PC-98 模拟器列表](https://www.zophar.net/pc98.html) [Zophar's Domain](https://www.zophar.net) 是一个历史可追溯至 1996 年的老网站，比东方Project的历史都要早，上面英文指南中的下载链接即指向了这个网站。 这个网站上还收录了许多其他模拟器，其中这些 PC-98 模拟器或许也值得一试。
...
用于改变游戏设定或是修复游戏内bug的附加文件，可能需要放入原游戏文件夹中并替换掉原游戏文件，尽量先备份游戏程序及数据（`score.dat` 等）再使用。
...
: 将 `thhyl\改版EXE\th15_ex_pmode.exe` 放到游戏目录下运行即可。本版本的EXTRA关卡只有完美模式。[对于中文版的支持参照原帖地址](http://tieba.baidu.com/p/4466644006)
...
: 更新至TH15正式版。将原作游戏的wav格式音源压缩为ogg格式，从而极大减少游戏文件夹的总体积。  
 ...
 当然不是指手机/掌机等等可用的版本。  
 [压缩好的全作游戏分流](https://cloud.lilywhite.cc/s/4ZUW?path=/东方Project/官方游戏/OGG版%20(节约硬盘空间%29)
THResHack
...
但对**修改游戏dat文件的汉化版**（例如市面上流行的“永夜抄汉化版+日文版”中，存在`th08.dat`和`th08c.dat`）可能出现跳转失效的情况。
[^cite_note-2]: [YouTube](https://www.youtube.com/channel/UChyVpooBi31k3xPbWYsoq3w)（需要翻墙）作者会在此发布更新公告
[^cite_note-3]: 此蓝奏云地址为Ack提供，随时可能失效。此前[原github仓库地址](https://github.com/ack7139/thprac)已随Ack注销github账号而失效。
[^cite_note-4]: vjoy可以创建一个虚拟手柄，游戏检测到虚拟手柄后，也不会再继续检测了。
[^cite_note-5]: 由外国人制作，特点是4面boss需要连续打18张（全机体的想起都需要打一遍）符卡，然后取消了波粒与十凶星的安定。
