...
- 资源方面，本游戏关卡模式及练习模式初始资源都是三残三b，不能调残或调b。游戏没有p点的设定，掉落道具只有蓝点、残机或bomb，掉落规则如下：击破boss符卡后掉落蓝点，若收取了该符卡（即符卡战中不miss，决死也不行，但是可以使用bomb），还会掉落残机或bomb（击破3n-2张符卡时才会掉残机，其他都是掉bomb）。收点线在屏幕上部1/4处（魔理沙为1/3，爱丽丝接到道具的范围较大），直接移过去就可以收资源。
- 分数主要依靠蓝点、擦弹、消弹等方式获取
- 详细内容可参考[游戏系统页面](./東方千夜帖~Thousand_Night_Anamnesis.-系统.md)
