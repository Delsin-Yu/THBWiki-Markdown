# 東方実在相_～_Dream_Logical_World/系统

<!-- source html: G:\repos\THBWiki-Markdown-Builder\THBWikiMarkdown\Temp\main\2\23\ns0%3A%E6%9D%B1%E6%96%B9%E5%AE%9F%E5%9C%A8%E7%9B%B8_%EF%BD%9E_Dream_Logical_World%2F%E7%B3%BB%E7%BB%9F.html -->
...
实在相包括六个可选择的双人团队。团队中先列出来的角色将是非集中时（松开Shift）操控的自机，而后列出来的则是集中时的。
  
  
判定点大小，自机速度和火力由以下格式显示：判定点直径/擦弹半径（像素）；非集中速度/集中速度（像素每帧）；非集中/集中火力每秒（假设灵力满格攻击全中）。在1.00灵力下，全角色秒伤均为60.
...
招出一个消弹的花之结界，消弹变成绿色得分物品。结界必须在射击时才能生效。
...
飞刀的角度随着咲夜的移动方向变化，并且会在板边反弹。
  
  
相位转换： **「Relative Velocity」** 
将时间缩短，提升咲夜和铃仙的攻击速度、攻击火力。
...
组合技能： **收集点得分更轻易延长相位转换时间。**  (每件物品延长0.133秒/8帧).
...
相位转换 **「量数化的祈祷」** 
没有即时效果，但在相位转换结束之后，玩家会获得奖励（但如果相位转换因掉残而中断则不会获得奖励）。相位转换维持时间越久奖励就越丰富。可能的奖励如下：
...
攻击时，箭矢会根据正邪的位置从板底或板上部发射。板底发射的箭矢会火力更强点。
  
  
相位转换： **「僭越者的舞弊」** 
  
  
没有即时效果，但相位转换结束之后的早苗和正邪会获得额外的无敌时间。相位转换维持得越久无敌时间就越长。计算公式如下：
  
  
无敌时间（帧）=（该相位转换获得的额外时间（帧）/60） ^ 0.7 x 60
...
弹种： **Brambly Rose Thorn** 
  
  
保持低速时，会从固定的位置发射弹幕。
  
  
相位转换： **「Subconscious Vigilance」** 
  
  
激活相位转换期间，另一转换的结界会来抵消任何伤害，你必须有大于1.00灵力来启动。根据你集中于否，妖梦和恋恋的相位转换均可激发此功能。
...
组合技能： **擦弹可以延长相位转换**  (在相位转换页面的数值X1.25).
...
给敌人的弹幕拍照，消成绿色得分物品。照片将拍摄最近的敌机，若目前屏幕上没有敌机，那就会在角色前方拍摄。在相位转换开启间，文文可以以3秒（180帧）冷却时间拍一张图片。
...
和其他东方弹幕射击游戏一样，Bombs是你在棘手情况下解围的好方法。不过本作实在相的独有特点为Bombs是名为相位转换的特殊符卡，这些符卡不能直接对敌人造成伤害，而是改变部分游戏机制来助你一臂之力
  
  
本作的Bomb系统与风神录和地灵殿类似，使用相位转换需要消耗1.00灵力。如果灵力值高于5.00，释放就没有损耗了（仅魔理沙和幽香组有最高6P）。当相位转换激活时，所有敌机弹幕都将被清除并转换为绿色得分道具，屏幕上所有物品都将飞向你（和得分线机制相同），角色获得2.5秒（150帧）的无敌时间，在此期间你依旧可以擦弹（不像你撞了复活后的无敌时间）。游戏背景变为紫色来指示相位转换已被激活。默认情况下一次相位转换持续6秒（360帧），紫色背景会从上到下逐渐消失，直到相位转换完全结束。
...
- 哲学级	0.021 秒 (1.28 帧)
- 神学级	0.019 秒 (1.12 帧)
- 艺术评判级  0.017 秒 (1.04 帧)
- 制作者级  0.0167 秒 (1 帧)

  
中弹或结束Boss战时会自动解除正在释放的相位转换，如果触发对话则会暂停其时间。你可以在一次相位转换的效果进行时再释放一个，不过它们不会叠加，第二个只会解除并覆盖第一个的效果。
  
  
中弹时按Bomb的机制与其他东方弹幕射击游戏一样，若玩家在中弹后0.133秒（8帧）间触发相位转换，则会触发其效果并规避中弹(决死)。若在中弹时已有相位转换被激活，决死时间将会扩大为0.5秒（30帧）。
恋的相位转换激活时，会在中弹时自动开启另一个相位转换并规避中弹（前提是灵力足够）
...
注：“最大得点”在下文中简称PIV
得分物品在自动收集（达成回收点条件/激活相位转换/对话）或于相位转换期间收集时值分最高。若不符合以上条件，则你收集它们的时候越靠近屏幕上方值分越多。计算公式如下：
  

- 得分 = PIV × ((-4 / 1,715) × 玩家当前高度（像素）+ (61 / 49))

  
该数值取最接近的10的倍数。而对特殊的工具与内在组来说，因为她们的收集点较低，所以公式为：
...
收集点高度和物品最高值高度重合。你可以轻松辨认收集时它们的点数是否为最大值（若为最大值文字将变为黄色）。对铃仙来说，若在相位转换时收集物品的点数为标准最大值X2时，文字将会变为紫色。
...
除了增幅你的灵力之外，大小增幅道具均在你灵力非满时值100分，在灵力叠满时值10,000分
  


## 符卡得分
  
Boss和道中会使用符卡攻击。当这发生时，游戏背景将会改变，并且该符卡的名字将会出现在屏幕的右上角。如果你在规定时间内NMNB收了这张卡（在符卡开启前触发的相位转换在该符卡的剩余时间不算Bomb），你将会得到符卡得分
...
当前可得分 = (初始可得分 / 规定时间（秒）) × 当前还剩下多少时间
  
  
在避弹时符中，Boss没有生命值，你也无法对其造成伤害，同样的，符卡得分也不会减少。若NMNB全避该时符则可获得符卡得分。
...
当相位转换耗尽时间/被另一个转换中断/击败Boss时（因miss而中断的相位转换不适用），玩家会获得相位转换得分并且屏幕上会出现紫色得分物品，物品的数量由相位转换持续时间决定。而相位转换得分则为PIV x 6.25 x 相位转换额外时间（你在默认的六秒之外延长的数值）（秒或0.083帧）。紫色物品造成的PIV得分则为相位转换额外时间（帧） x 1.25 ^ 1.3，再乘于根据难度的以下数值：
...
当玩家在开启相位转换时中弹并在正常的8帧决死时间内按X，相位转换得分将以“之前开启的相位转换被新的所替代”的方式计算，而若玩家在超出8帧的时间内按X，该得分将会线性降低直到超出相位转换所提供的额外决死时长，在决死时长即将结束时降为0，若新的相位转换在延长决死时间触发，相位转换得分将会以红字显示。若相位转换因击败Boss而中断，得分将以“该相位转换耗尽其所有时长”计算。
...
当你掉残的时，任何当前开启的相位转换都会停止，屏上所有弹幕清除，获得5秒（300帧）的无敌时间，在此期间无法擦弹。你会失去所有灵力，但屏上将仍有你可以捡回来的一些P点道具，大道具的数量为掉残之前灵力数量的整数-1。这个数值也会由你还剩下多少残机而变化：如果剩下3或以上数量的残机，大道具的数量大于等于2；如果还剩下2或1残机，大道具的数量大于等于3；若没有残机剩余，大道具的数量将会叠满。
